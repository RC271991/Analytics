...
shinyServer(function(input, output, session) {
  ...
  #From Here we will begin our group authenication
  ...
  #Connecting api to gitlab to verify if the user is in the group with access_token recieved from the 'user authenication' process.
  ...
  observe({
    #If user has credientials, then the user gets redirected to your dashboard. If not, the user gets sent to a page
    #asking them to contact the adminstrator of the app to include them into the gitlab's group list.
    
    if (length(group) == 0){ #If user is not in group then the length of group will be 0 because the response from gitlab will be empty
